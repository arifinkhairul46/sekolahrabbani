...
@section('content')
            ...
            @foreach ($cart_detail as $item)
                    ...
                    <div class="row-card" style="justify-content: center; align-items:center">
                        ...
                        <div class="deskripsi" style="max-width: 225px">
                            <p class="mb-0" style="font-size: 14px"><b> {{$item->nama_produk}} Design by {{$item->nama_siswa}}, 
                                size {{$ukuran}}, {{$item->kategori}}, {{$item->warna}}, {{$item->template}} </b> 
                            </p>
                            ...
                            <div class="input-group" style="border: none;">
                                ...
                                <form action="{{route('merchandise-cart.delete', $item->id)}}" method="post" style="margin-left:auto">
                                    @csrf @method('DELETE')
                ...
                @else
                    <div class="row-card" style="justify-content: center; align-items:center">
                        ...
                        <div class="deskripsi" style="max-width: 225px">
                            <p class="mb-0" style="font-size: 14px"><b> {{$item->nama_produk}} </b> </p>
                            @if ($item->diskon == 0 || $item->diskon == null)
                ...
                @endif
            <?php $total += (($item['harga_awal'] * $item['quantity']) - ($item['diskon']/100 * $item['harga_awal'] * $item['quantity'])); ?>
            @endforeach
    ...
    @else
        <div class="center-cart">
            ...
            <a href="{{route('palestine.merchandise')}}" style="text-decoration: none">
    ...
    <script>
        ...
        function oncheck(id) {
            ...
            var url = "{{ route('merchandise-cart-select.update', ":id") }}";
            url = url.replace(':id', id);
        ...
        function getCheckedBoxes(main) {
            ...
            $.ajax({
                type: "PUT",
                url: "{{route('merchandise-select-all-cart')}}",
                data: {
                    checks: checks,
                    ids: ids,
                    _token: '{{csrf_token()}}'
